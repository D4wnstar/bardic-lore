...
/*
This code is taken directly from the source code of rodio 0.20.1.
The original version of this code was licensed under the Apache 2 license
...
use rodio::source::{Empty, EmptyCallback, SeekError, Source, Zero};
use rodio::Sample;
...
pub struct SourcesQueueInput<S> {
    next_sounds: Mutex<Vec<(Sound<S>, bool, SignalDone)>>,

    // See constructor.
    keep_alive_if_empty: AtomicBool,
}

impl<S> Debug for SourcesQueueInput<S>
where
    S: Sample + Send + 'static,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let sounds = self.next_sounds.lock().unwrap();
        write!(f, "SourcesQueueInput {{ next_sounds: [ ")?;
    ...
    S: Sample + Send + 'static,
{
    ...
    #[inline]
    pub fn append<T>(&self, source: T, is_callback: bool)
    where
        T: Source<Item = S> + Send + 'static,
    {
        self.next_sounds
            .lock()
            .unwrap()
            .push((Box::new(source) as Box<_>, is_callback, None));
    }
    ...
    #[inline]
    pub fn append_with_signal<T>(&self, source: T, is_callback: bool) -> Receiver<()>
    where
        T: Source<Item = S> + Send + 'static,
    {
        let (tx, rx) = channel();
        self.next_sounds
            .lock()
            .unwrap()
            .push((Box::new(source) as Box<_>, is_callback, Some(tx)));
        rx
    }

    pub fn insert<T>(&self, index: usize, source: T, is_callback: bool) -> Receiver<()>
    where
        T: Source<Item = S> + Send + 'static,
    {
        let (tx, rx) = channel();
        let new_sound = (Box::new(source) as Box<_>, is_callback, Some(tx));
        self.next_sounds.lock().unwrap().insert(index, new_sound);

        return rx;
    }

    pub fn remove(&self, index: usize) {
        self.next_sounds.lock().unwrap().remove(index);
    }
